...
The random generator covers hazards by volume; these cover the ones too
specific to appear by chance, or whose distances have to be exact.  Both go
through the same build-and-diff path (test_cosim_a5.py), and the whole
commit log is compared.  Each case in the .S files names the distance it
exercises; the mutation harness in tb/mutate/ is what proves each one
actually contains the hazard it claims.
"""
import argparse
import os
import subprocess
...
def compile_s(path, tmp, name):
    ...
    r = subprocess.run(
        # a14_muldiv.S is written in M instructions; the other three programs
