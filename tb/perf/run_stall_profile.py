...
FIELDS = ("cycle", "retired", "id_stall", "ex_stall", "redirect",
...
def main() -> int:
    ...
    if a.selftest:
        ...
        for f in (1, 2, 3, 4, 5):
            ...
            hit = d["residual"] != 0 or mis_resid != 0
            print("  fault %d (%-46s) residual %+8d  mispredict %+6d  %s"
                  % (f, why[f], d["residual"], mis_resid,
                     "CAUGHT" if hit else "ESCAPED"))
            if not hit:
                problems.append("selftest fault %d escaped both checks" % f)
