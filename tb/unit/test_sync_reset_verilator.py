#!/usr/bin/env python3
"""Build and run the rvntt_sync_reset Verilator testbench. Exits nonzero on failure."""
import os, subprocess, sys, tempfile

ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
RTL  = os.path.join(ROOT, "rtl/common/rvntt_sync_reset.sv")
...
def main():
    ...
    exe = os.path.join(build, "Vrvntt_sync_reset")
    r = subprocess.run([exe], stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    out = r.stdout.decode("utf-8", "replace")
    print(out.strip())
    return 0 if (r.returncode == 0 and "SYNC_RESET_TB_OK" in out) else 1
